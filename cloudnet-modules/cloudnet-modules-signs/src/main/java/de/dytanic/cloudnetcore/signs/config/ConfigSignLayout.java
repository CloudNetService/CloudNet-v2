...
public class ConfigSignLayout {
	...
	public ConfigSignLayout() {
		if (!Files.exists(path)) {
			SignLayoutConfig signLayoutConfig = new SignLayoutConfig(true, false, 1D, 0.8D,
			...
			new Document().append("layout_config", Document.GSON.toJson(signLayoutConfig, TypeToken.get(SignLayoutConfig.class).getType())).saveAsConfig(path);
		}
	}

	public ConfigSignLayout saveLayout(SignLayoutConfig signLayoutConfig) {
		Document document = Document.loadDocument(path);
		document.append("layout_config", Document.GSON.toJson(signLayoutConfig, TypeToken.get(SignLayoutConfig.class).getType()));
	...
	public de.dytanic.cloudnet.lib.serverselectors.sign.SignLayoutConfig loadLayout() {
		...
		if (injectable) {
			document.append("layout_config", Document.GSON.toJson(signLayoutConfig,TypeToken.get(SignLayoutConfig.class).getType())).saveAsConfig(path);
		}
