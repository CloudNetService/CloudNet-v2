...
public class ConfigMobs implements ILoader<MobConfig> {
	...
	public ConfigMobs() {
		if (!Files.exists(path)) {
			MobConfig mobConfig = new MobConfig(54, 10, new MobItemLayout(388, "EMERALD", 0,
			...
			new Document().append("mobConfig", Document.GSON.toJson(mobConfig, TypeToken.get(MobConfig.class).getType())).saveAsConfig(path);
		}
	}

	@Override
	public MobConfig load() {
		return Document.loadDocument(path).getObject("mobConfig", TypeToken.get(MobConfig.class).getType());
	}
