    ...
    public void handleInput(Document data, PacketSender packetSender)
    {
        ...
        if (data.getString("type").equals(DefaultType.BUKKIT.name()))
        {
            ...
            try
            {
                ...
                {
                    if (!Files.exists(Paths.get("local/templates/" + serverGroup.getName() + "/" + template.getName())))
                {
                    System.out.println("Creating GroupTemplate for " + serverGroup.getName() + " " + template.getName() + "...");
                    Files.createDirectories(Paths.get("local/templates/" + serverGroup.getName() + "/" + template.getName() + ""));
                }

                    if (!Files.exists(Paths.get("local/templates/" + serverGroup.getName() + "/" + template.getName() + "/server.properties")))
                    {
                        FileCopy.insertData("files/server.properties", "local/templates/" + serverGroup.getName() + "/" + template.getName() + "/server.properties");
                    }

                    if (!Files.exists(Paths.get("local/templates/" + serverGroup.getName() + "/" + template.getName() + "/plugins")))
                    {
                        new File("local/templates/" + serverGroup.getName() + "/" + template.getName() + "/plugins").mkdir();
                    }
                }
            ...
            if (serverGroup.getServerType().equals(ServerGroupType.CAULDRON))
            {
                for (Template template : serverGroup.getTemplates())
                {
                    if (!Files.exists(Paths.get("local/templates/" + serverGroup.getName() + "/" + template.getName() + "/cauldron.jar")))
                    {
                        try
                        {
                            System.out.println("Downloading cauldron.zip...");
                            File file = new File("local/templates/" + serverGroup.getName() + "/" + template.getName() + "/cauldron.zip");
                            ...
                            while (entryEnumeration.hasMoreElements())
                            {
                                ...
                                {
                                    extractEntry(zip, entry, "local/templates/" + serverGroup.getName() + "/" + template.getName());
                                }
                            }
            ...
            if (serverGroup.getServerType().equals(ServerGroupType.GLOWSTONE))
            {
                for (Template template : serverGroup.getTemplates())
                {
                    Path path = Paths.get("local/templates/" + serverGroup.getName() + "/" + template.getName() + "/glowstone.jar");
