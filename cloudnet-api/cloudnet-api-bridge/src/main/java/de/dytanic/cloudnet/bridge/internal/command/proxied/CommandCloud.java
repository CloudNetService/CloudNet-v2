...
public final class CommandCloud extends Command implements TabExecutor {
        ...
        if (args.length > 2) {
            if (args[0].equalsIgnoreCase("cmds") && commandSender.hasPermission("cloudnet.command.cloud.commandserver")) {
                if (CloudProxy.getInstance().getCachedServers().containsKey(args[1])) {
                    ...
                    commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() +
        ...
        switch (args.length) {
            case 1:
                if (args[0].equalsIgnoreCase("whitelist")) {
                    commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "Whitelisted players from " + CloudProxy.getInstance().getProxyGroup().getName()));
                    ...
                    commandSender.sendMessage(TextComponent.fromLegacyText("CloudNet " + NetworkUtils.class.getPackage().getSpecificationVersion() + " #" + NetworkUtils.class.getPackage().getImplementationVersion() + " by Dytanic"));
                    return;
                    ...
                    for (WrapperInfo cnsInfo : CloudAPI.getInstance().getWrappers()) {
                        commandSender.sendMessage(TextComponent.fromLegacyText("§8[§7" + cnsInfo.getServerId() + "§8/§7" + cnsInfo.getHostName() + "§8] §7Cores: " + cnsInfo.getAvailableProcessors()));
                        maxMemory = maxMemory + cnsInfo.getMemory();
                    }
                    commandSender.sendMessage(TextComponent.fromLegacyText(NetworkUtils.SPACE_STRING));
                    for (ProxyInfo simpleProxyInfo : CloudAPI.getInstance().getProxys()) {
                        commandSender.sendMessage(TextComponent.fromLegacyText("§8[§c" + simpleProxyInfo.getServiceId().getServerId() + "§8] §8(§e" + simpleProxyInfo.getOnlineCount() + "§8) : §7" + simpleProxyInfo.getMemory() + "MB"));
                    ...
                    for (ProxyInfo proxy : CloudAPI.getInstance().getProxys()) {
                        commandSender.sendMessage(TextComponent.fromLegacyText("§7- " + (proxy.isOnline() ? "§e" : "§c") + proxy.getServiceId().getServerId() + " §8(§e" + proxy.getOnlineCount() + "§8) "));
                    }
                    return;
                    ...
                    for (CloudPlayer playerWhereAmI : CloudAPI.getInstance().getOnlinePlayers()) {
                        commandSender.sendMessage(TextComponent.fromLegacyText("§7- §e" + playerWhereAmI.getName() + " §7on §e" + playerWhereAmI.getServer() + NetworkUtils.SLASH_STRING + playerWhereAmI.getProxy()));
                    }
                    return;
            ...
            case 2:
                if (args[0].equalsIgnoreCase("toggle")) {
                    switch (args[1].toLowerCase()) {
                        case "autoslot": {
                            if (commandSender.hasPermission("cloudnet.command.cloud.autoslot")) {
                                ...
                                commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The autoslot state was updated."));
                            }
                        }
                        return;
                        case "maintenance": {
                            if (commandSender.hasPermission("cloudnet.command.cloud.maintenance")) {
                                ...
                                CloudAPI.getInstance().updateProxyGroup(proxyGroup);
                                commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The maintenance state was updated."));
                            }
                            return;
                        }
                    }
                    ...
                    if (CloudProxy.getInstance().getCachedServers().containsKey(args[1]) ||
                            CloudAPI.getInstance().getProxys().stream().anyMatch(proxyInfo -> proxyInfo.getServiceId().getServerId().equalsIgnoreCase(args[1]))) {
                        ...
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The log is dynamic and will be deleted on 10 minutes"));
                    } else {
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The server doesn't exist."));
                    }
                    return;
                    ...
                    if (isNumber(args[1])) {
                        ...
                        CloudAPI.getInstance().updateProxyGroup(proxyGroup);
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The maximum onlinecount was updated."));
                    } else {
                    ...
                    if (CloudAPI.getInstance().getCloudNetwork().getServerGroups().containsKey(args[1])) {
                        ...
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The group doesn't exist."));
                    }
                    return;
                    ...
                    if (CloudProxy.getInstance().getCachedServers().containsKey(args[1])) {
                        ...
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() +
                                "The information was sent to the cloud"));
                    } else if (CloudAPI.getInstance().getProxys().stream().anyMatch(proxyInfo -> proxyInfo.getServiceId().getServerId().equalsIgnoreCase(args[1]))) {
                        CloudAPI.getInstance().stopProxy(args[1]);
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() +
                                "The information was sent to the cloud"));
                    } else {
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The specified server isn't online."));
                    }
                    return;
                    ...
                    if (CloudAPI.getInstance().getServerGroupMap().containsKey(args[1])) {
                        ...
                        CloudProxy.getInstance().getCachedServers().values().stream()
                                .filter(serverInfo -> serverInfo.getServiceId().getGroup() != null &&
                                        serverInfo.getServiceId().getGroup().equalsIgnoreCase(args[1]))
            ...
            case 3:
                if (args[0].equalsIgnoreCase("copy")) {
                    if (!CloudProxy.getInstance().getCachedServers().containsKey(args[1])) {
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The server doesn't exists"));
                        return;
                    }

                    CloudAPI.getInstance().copyDirectory(CloudProxy.getInstance().getCachedServers().get(args[1]),
                            args[2]);
                    commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The wrapper tried to copy the directory..."));
                ...
                if (args[0].equalsIgnoreCase("toggle")) {
                    if (args[1].toLowerCase().equals("maintenance")) {
                        if (commandSender.hasPermission("cloudnet.command.cloud.maintenance")) {
                            ...
                            commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + "The maintenance will be changed in " + args[2] + " seconds"));
                        }
                        return;
                    }
                    return;
                ...
                if (args[0].equalsIgnoreCase("start") && commandSender.hasPermission("cloudnet.command.cloud.start")) {
                    if (CloudAPI.getInstance().getServerGroupMap().containsKey(args[1])) {
                        if (NetworkUtils.checkIsNumber(args[2])) {
                            ...
                            ServerGroup serverGroup = CloudAPI.getInstance().getServerGroup(args[1]);
                            Optional<Template> template = serverGroup.getTemplates().stream().filter(t -> t.getName().equalsIgnoreCase(args[2])).findFirst();
                            template.ifPresent(value -> {
                                CloudAPI.getInstance().startGameServer(CloudAPI.getInstance().getServerGroupData(args[1]), new ServerConfig(false, "extra", new Document(), System.currentTimeMillis()), true, value);
                                commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() +
                                        "The information was sent to the cloud"));
                            });
                        }
                    ...
                    if (args[1].equalsIgnoreCase("add")) {
                        ...
                        if (proxyGroup.getProxyConfig().getWhitelist().contains(args[2])) {
                            commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + " The user " + args[2] + " is already on the whitelist."));
                            return;
                        }
                        proxyGroup.getProxyConfig().getWhitelist().add(args[2]);
                        CloudAPI.getInstance().updateProxyGroup(proxyGroup);
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + " You added " + args[2] + " to the whitelist of the maintenance mode."));
                    } else if (args[1].equalsIgnoreCase("remove")) {
                        ProxyGroup proxyGroup = CloudProxy.getInstance().getProxyGroup();
                        proxyGroup.getProxyConfig().getWhitelist().remove(args[2]);
                        CloudAPI.getInstance().updateProxyGroup(proxyGroup);
                        commandSender.sendMessage(TextComponent.fromLegacyText(CloudAPI.getInstance().getPrefix() + " You removed " + args[2] + " from the whitelist of the maintenance mode."));
                    }
                }
                break;
            case 4:
    ...
    private List<String> getProxiesAndServers() {
        LinkedList<String> groups = CloudAPI.getInstance().getProxys().stream()
                .map(ProxyInfo::getServiceId).map(ServiceId::getServerId).collect(Collectors.toCollection(LinkedList::new));
