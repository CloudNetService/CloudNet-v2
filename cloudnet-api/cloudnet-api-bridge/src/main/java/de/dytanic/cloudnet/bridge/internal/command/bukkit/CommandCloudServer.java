...
public final class CommandCloudServer implements CommandExecutor, TabExecutor {
    ...
    private static MobSelector.MobImpl findMobWithName(String arg) {
        return MobSelector.getInstance().getMobs().values().stream()
    ...
    private boolean setItem(CommandSender commandSender, String[] args) {
        ...
        if (mob != null) {
            ...
            if (material != null) {
                ...
                commandSender.sendMessage(CloudAPI.getInstance().getPrefix() + "You set the item for \"" + args[1] + "\" the material \"" + material.name() + '"');
            }
            return true;
        }
        return false;
    ...
    private boolean removeSign(CommandSender commandSender, Player player) {
        if (checkSignSelectorActive(commandSender)) return true;
    ...
    private boolean createSign(CommandSender commandSender, String[] args, Player player) {
        if (checkSignSelectorActive(commandSender)) return false;
        ...
        try {
            ...
            if (MobSelector.getInstance().getMobs().values().stream().noneMatch(
                ...
                ServerMob serverMob = new ServerMob(UUID.randomUUID(), stringBuilder.substring(0, stringBuilder.length() - 1), args[2], entityType.name(), args[3],
                        -1, material.name(), args[5].equalsIgnoreCase("true"),
