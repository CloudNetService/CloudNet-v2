...
public class PacketInMobSelector extends PacketInHandlerDefault {
    ...
    public void handleInput(Document data, PacketSender packetSender) {
        ...
        if (MobSelector.getInstance() != null) {
            Bukkit.getScheduler().runTask(CloudServer.getInstance().getPlugin(), () -> {
                ...
                MobSelector.getInstance().setMobs(MapWrapper.transform(filteredMobs, key -> key,
                    key -> {
                        ...
                        MobSelector.MobImpl mob = new MobSelector.MobImpl(key.getUniqueId(), key,
                            entity, MobSelector.getInstance().create(mobConfig, key),
                            new HashMap<>(), armorStand);
                ...
                Bukkit.getScheduler().runTaskAsynchronously(CloudServer.getInstance().getPlugin(),
                    () -> {
                        for (ServerInfo serverInfo : MobSelector.getInstance().getServers()
                            .values()) {
            ...
            MobSelector.getInstance().setMobs(new HashMap<>());
            Bukkit.getScheduler().runTask(CloudServer.getInstance().getPlugin(),
                () -> MobSelector.getInstance().setMobs(MapWrapper.transform(filteredMobs,
                    key -> key, key -> {
                        MobSelector.getInstance().toLocation(key.getPosition()).getChunk().load();
                        Entity entity = MobSelector.getInstance().toLocation(key.getPosition())
                            .getWorld()
                        ...
                        entity.setCustomName(ChatColor.translateAlternateColorCodes('&',
                            key.getDisplay() + NetworkUtils.EMPTY_STRING));
