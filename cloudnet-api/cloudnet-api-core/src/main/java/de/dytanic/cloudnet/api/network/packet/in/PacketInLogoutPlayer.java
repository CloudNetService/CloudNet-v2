	...
	public void handleInput(Document data, PacketSender packetSender) {
		...
		if (cloudPlayer != null) {
			if (CloudAPI.getInstance() != null) {
				CloudAPI.getInstance().getNetworkHandlerProvider().iterator(obj -> obj.onPlayerDisconnectNetwork(cloudPlayer));
			}
		} else {
			UUID uuid = data.getObject("uniqueId", UUID.class);
			if (CloudAPI.getInstance() != null) {
				CloudAPI.getInstance().getNetworkHandlerProvider().iterator(obj -> obj.onPlayerDisconnectNetwork(uuid));
			}
		}
	}
}