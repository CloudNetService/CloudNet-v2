    ...
    public void handleInput(Document data, PacketSender packetSender)
    {
        if(!data.contains("wrapper"))
        {
            CloudNet.getInstance().startProxy(CloudNet.getInstance().getProxyGroups().get(data.getString("group")),
                    data.getInt("memory"), data.getObject("processParameters", new TypeToken<String[]>(){}.getType()), data.getString("url"),
                    data.getObject("plugins", new TypeToken<Collection<ServerInstallablePlugin>>(){}.getType()), data.getDocument("properties"));
        }
        else
        {
