...
public class PacketAPIInGetServers extends PacketAPIIO {
    ...
    public void handleInput(Document data, PacketSender packetSender)
    {
        ...
        {
            Collection<ServerInfo> proxyInfos = CollectionWrapper.transform(CloudNet.getInstance().getServers(data.getString("group")), new Function<MinecraftServer, ServerInfo>() {
                @Override
                public ServerInfo apply(MinecraftServer key)
                {
        ...
        {
            Collection<ServerInfo> proxyInfos = CollectionWrapper.transform(CloudNet.getInstance().getServers().values(), new Function<MinecraftServer, ServerInfo>() {
                @Override
                public ServerInfo apply(MinecraftServer key)
                {
                    return key.getServerInfo();
