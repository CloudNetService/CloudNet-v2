	...
	public void handleInput(Document data, PacketSender packetSender) {
		...
		if (defaultType.equals(DefaultType.BUKKIT)) {
			if (data.contains("serverId")) {
				...
				CloudNet.getInstance().getNetworkManager().sendAll(new PacketOutCustomSubChannelMessage(channel, message, document), networkComponent -> networkComponent instanceof MinecraftServer || networkComponent instanceof CloudServer);
			}
		} else {
			if (data.contains("serverId")) {
