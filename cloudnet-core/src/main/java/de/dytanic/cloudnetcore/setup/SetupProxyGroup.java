...
public class SetupProxyGroup {
    ...
    public SetupProxyGroup(CommandSender commandSender, String name)
    {
        ...
        Setup setup = new Setup()
                ...
                .request(new SetupRequest("memory", "How many MB of RAM should the proxy group have?", "Specified memory is invalid", SetupResponseType.NUMBER, key -> NetworkUtils.checkIsNumber(key) && Integer.parseInt(key) > 64))
                .request(new SetupRequest("startport", "What's the starting port of the proxygroup?", "Specified starting port is invalid", SetupResponseType.NUMBER, key -> NetworkUtils.checkIsNumber(key) && Integer.parseInt(key) > 128 && Integer.parseInt(key) < 70000))
                ...
                .request(new SetupRequest("mode", "Should the group be STATIC or DYNAMIC?", "Group mode is invalid", SetupResponseType.STRING, key -> key.equalsIgnoreCase("STATIC") || key.equalsIgnoreCase("DYNAMIC")))
                .request(new SetupRequest("template", "What is the backend of the group default template? [\"LOCAL\" for the wrapper local | \"MASTER\" for the master backend]", "String is invalid", SetupResponseType.STRING, key -> key.equals("MASTER") || key.equals("LOCAL")))
                .request(new SetupRequest("wrapper", "Which wrappers should be used for this group?", "String is invalid", SetupResponseType.STRING, key -> {
