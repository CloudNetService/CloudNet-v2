...
public class SetupServerGroup {
    ...
    public SetupServerGroup(CommandSender commandSender, String name)
    {
        ...
        Setup setup = new Setup()
                .setupCancel(() -> System.out.println("Setup cancelled!"))
                .setupComplete(data -> {
                ...
                .request(new SetupRequest("memory", "How many MB RAM should the server group have?", "Specified Memory is invalid", SetupResponseType.NUMBER, key -> NetworkUtils.checkIsNumber(key) && Integer.parseInt(key) > 64))
                .request(new SetupRequest("startup", "How many servers should always be online?", "Specified startup count is invalid", SetupResponseType.NUMBER, key -> true))
                .request(new SetupRequest("percent", "How full does the server have to be until a new server is started? (In Percent)?", "Specified percent count is invalid", SetupResponseType.NUMBER, key -> NetworkUtils.checkIsNumber(key) && Integer.parseInt(key) <= 100))
                .request(new SetupRequest("mode", "Which server group mode should be used? [STATIC, STATIC_LOBBY, LOBBY, DYNAMIC]", "Specified server group mode is invalid", SetupResponseType.STRING, key -> key.equalsIgnoreCase("STATIC") || key.equalsIgnoreCase("STATIC_LOBBY") || key.equalsIgnoreCase("LOBBY") || key.equalsIgnoreCase("DYNAMIC")))
                .request(new SetupRequest("type", "Which servergroup type should be used? [BUKKIT, CAULDRON, GLOWSTONE]", "Specified group type is invalid", SetupResponseType.STRING, key -> key.equals("BUKKIT") || key.equals("GLOWSTONE") || key.equals("CAULDRON")))
                .request(new SetupRequest("template", "What is the backend of the group default template? [\"LOCAL\" for the wrapper local | \"MASTER\" for the master backend]", "Specified string is invalid", SetupResponseType.STRING, key -> key.equals("MASTER") || key.equals("LOCAL")))
                ...
                .request(new SetupRequest("wrapper", "Which wrappers should be used for this group?", "Specified string is invalid", SetupResponseType.STRING, key -> {
